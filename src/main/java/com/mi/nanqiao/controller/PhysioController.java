...
public class PhysioController {
    ...
    public R<PhysioDTO> findById(@PathVariable Integer id){
        Assert.notNull(id,"理疗师ID不能为空");
        PhysioDTO physio = physioService.queryById(id);
        return R.ok(physio);
    }
    ...
    public R<List<Physio>> findByProjectId(@PathVariable("id") Integer id) {
        ...
        List<Physio> physioList = physioService.queryByProject(id);
